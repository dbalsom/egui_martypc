...
pub struct WebOptions {
    ...
    /// Whether a raw web keyboard event should have `preventDefault` called on it or not.
    ///
    /// This runs before the event is translated to an [`egui::Event`], so it can inspect the
    /// browser [`KeyboardEvent.code`](https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/code).
    ///
    /// Defaults to false.
    pub should_prevent_default_for_raw_key: Box<dyn Fn(&crate::WebKeyboardEvent) -> bool>,
...
impl Default for WebOptions {
    fn default() -> Self {
        Self {
            ...
            should_prevent_default_for_raw_key: Box::new(|_| false),
